...
import { sampleWithRequiredData } from '../message-chat.test-samples';
...
describe('MessageChat Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: MessageChat;
  let fixture: ComponentFixture<MessageChat>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(MessageChat);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 6277 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.messageChats()[0]).toEqual(expect.objectContaining({ id: 6277 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 6277 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.messageChats()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 6277 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vitest.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to messageChatService', () => {
      const entity = { id: 6277 };
      vitest.spyOn(service, 'getMessageChatIdentifier');
      const id = comp.trackId(entity);
      expect(service.getMessageChatIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.messageChatsParams()).toMatchObject(expect.objectContaining({ sort: ['id,desc'] }));
  });
